...
pro my_eq2hor, ra, dec, jd, alt, az, ha, lat=lat, lon=lon, WS=WS, obsname=obsname,$
     B1950 = B1950, verbose=verbose, precess_=precess_, nutate_=nutate_, $
                refract_ = refract_, aberration_ = aberration_,  $
                altitude = altitude, lst=lst, _extra= _extra
...
if n_elements(lat) eq 0 then lat = 19.8261111d ; (btw, this is the declination
                                            ; of the zenith)
if n_elements(lon) eq 0 then lon = -155.473366d
if n_elements(altitude) eq 0 then altitude = 4072. ; [meters]
...
if precess_ then begin
    print,"WARNING: PRECESSING IN EQ2HOR"
...
getaltaz,dec_,lat,ha/15.,galt,gaz
total_diff = sqrt( (galt-alt)^2 + (gaz-az)^2 )
if max(total_diff) gt 1./3600. then begin
    message,"Error: hadec2altaz and getaltaz differed by "+strc(max(total_diff)*3600.)+" arcseconds ("+strc(max(total_diff))+" degrees)",/info
    az=gaz
    alt=galt
endif
...
;  ADAM'S REVISION:  NO REFRACTION CORRECTION!
