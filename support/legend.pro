...
;                      curbox,x,y,xsz,ysz
  ...
  if keyword_set(spos) then begin
    if not keyword_set(norm) then begin
      ...
      ysz=ysize
    endelse
    if keyword_set(!p.charsize) then xsz=xsz*!p.charsize
    if keyword_set(!p.charsize) then ysz=ysz*!p.charsize
    case spos of
...
symsize=charsize
dx=charsize*cnvrtx
dy=1.1*cnvrty*(y1-y0)/ysize ; dy=1.1*charsize*cnvrty
...
jlbl=0
stdspc=lenstr(' ')*charsize

for i=0,n-1 do begin
  ylab=yy-.3*charsize*cnvrty
  ...
  if titles(i) eq '' then begin
    ...
    xyouts,xlab,ylab,labls(i),color=fgc,charsize=charsize,/norm
    jlbl=jlbl+1
...
;    stop
    if just(i) eq 'f' then begin
      ...
      for j=0,nwords-1 do begin 
        xyouts,xtitle,ylab,tstr(j),color=fgc,charsize=charsize,/norm 
        xtitle=xtitle+dspc+lngth(j)
      endfor
    endif else begin  
      xyouts,xtitle,ylab,titles(i),color=fgc,charsize=charsize,$
