...
pro measure_flux,savefile,fitsfile,simmapfile,flux_recov=flux_recov,flux_input=flux_input,$
    ...
    for i=0,numsrc-1 do begin ; recall xwidth,ywidth = gaussian sigma
        flux_recov[i] = measure_box(map,xcen[i],ycen[i],xwidth[i]*2.35,ywidth[i]*2.35,/ellipse)
        flux_input[i] = measure_box(simmap,xcen[i],ycen[i],xwidth[i]*2.35,ywidth[i]*2.35,/ellipse)
    endfor
