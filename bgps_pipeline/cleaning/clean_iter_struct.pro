...
pro clean_iter_struct,bgps,mapstr,niter=niter,$
    ...
    if keyword_set(scale_acb) then scalebolos = bgps.ac_bolos*bgps.scalearr $
        else scalebolos = bgps.ac_bolos
    bgps.atmosphere = scalebolos - bgps.astrosignal 
...
; NEVER do this, the results are terrible    if keyword_set(boloflat) then new_astro = bolo_flat(new_astro)
    ...
    if niter[i] gt 0 then bgps.noise = nantozero( scalebolos - first_sky - pca_atmo - bgps.astrosignal ) $ 
        else  bgps.noise = nantozero( scalebolos - first_sky - bgps.astrosignal )
