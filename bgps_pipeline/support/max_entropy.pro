...
pro max_entropy, data, psf, deconv, multipliers, FT_PSF=psf_ft, NO_FT=noft, $
	...
	deconv = exp( convolve( multipliers, psf, FT_PSF=psf_ft, $
						 /CORREL, NO_FT=noft ) )
        tophalf = where( deconv gt mean(deconv) )
        if n_e(tophalf) le 1 then tophalf=where(finite(deconv)) 
	totd = total( data[tophalf] )
	deconv = deconv * ( totd/total( deconv[tophalf] ) )

	Re_conv = convolve( deconv, psf, FT_PSF=psf_ft, NO_FT=noft )

        tophalf = where( re_conv gt mean(re_conv) )
        if n_e(tophalf) le 1 then tophalf=where(finite(re_conv)) 
	totd = total( data[tophalf] )
	scale = total( Re_conv[tophalf] )/totd
