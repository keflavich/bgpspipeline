...
; If no lat or lng entered, use Caltech Submillimeter Observatory values
if n_elements(lat) eq 0 then lat = 19.8261111d
; (btw, this is the declination of the zenith)
if n_elements(lon) eq 0 then lon = -155.473366d
...
; NO! Says Adam.  if refract_ then alt_ = co_refract(alt_, altitude=altitude, _extra=_extra)
...
if keyword_set(lst) then LAST=lst*15.
...
; ERROR CHECKING (forced!)
getradec,alt_,az_,lat,gha,gdec
gra = (last-(gha)*15.+360.) mod 360.
total_diff = sqrt( (gra-ra)^2 + (gdec-dec)^2 )
if max(total_diff) gt 1./3600. then begin
    if max(total_diff) lt 1./3600. then $
        message,"Error: altaz2hadec and getradec differed by "+strc(max(total_diff*3600.))+" arcseconds ("+strc(max(total_diff))+" degrees)",/info $
        else message,"Error: altaz2hadec and getradec differed by "+strc(max(total_diff*3600.))+" arcseconds ("+strc(max(total_diff))+" degrees)" 
    ra=gra
    dec=gdec
endif
