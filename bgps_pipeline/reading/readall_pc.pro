...
; 9/5/08 changed to read from file    if ~keyword_set(mvperjy) then mvperjy = [-0.00333379,-2.92617,6.97269]
    ...
    for i=0,n_e(filelist)-1 do begin
        ...
        if size(distcor,/type) eq 7 then begin ; if distcor is a string specifying a beam locations file...
            if distcor eq 'lookup' then lookup_distmap,filename,bolonum=bolonum,bolodist=bolodist,boloang=boloang,boloerr=err $
            else readcol,distcor,bolonum,bolodist,boloang,err,comment="#;",format="(I, F, F, F)",/silent
