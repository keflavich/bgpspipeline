pro cataloger, filelist, atlas = atlas, $
  ...
  if n_elements(filelist) eq 0 then filelist = 'v2.0_July28_reproj_list.txt'
  if n_elements(boundary_file) eq 0 then boundary_file = 'bounds.txt'

  if n_elements(directory) eq 0 then directory = '/Volumes/disk2/data/bgps/releases/v2.0/maps/'
