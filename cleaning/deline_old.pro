...
function deline, bolo_array, sample_interval, scans_info=scans_info,$
    lines=lines, flat=flat

    if ~keyword_set(scans_info) then begin
        return,deline_new(bolo_array,sample_interval,lines=lines,flat=flat)
    endif
    ...
    for s=0,nscans-1 do begin
        ...
        ; Fourier transform
        ftac = fft(bolo_array[*,lb:ub],dim=2,/double)
