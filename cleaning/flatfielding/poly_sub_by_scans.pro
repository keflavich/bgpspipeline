...
;            temp[b,lb:ub] = poly_sub(temp[b,lb:ub],sigmareject=1,iterate_sigmareject=1,polysub_order=2)  ; old version: use sigma-rejection and a line fit
            temp[b,lb:ub] = poly_sub(temp[b,lb:ub],sigmareject=3,iterate_sigmareject=3,order=polysub_order)  ; new version (8/11/08): I think a higher polysub_order polynomial will work better: it won't take out sources
