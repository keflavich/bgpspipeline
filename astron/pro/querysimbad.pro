PRO QuerySimbad, name, ra, de, id, Found = found, NED = ned, ERRMSG = errmsg, $
    Verbose = verbose, CADC = cadc, CFA=cfa
;+
; NAME: 
;   QUERYSIMBAD
;
...
;
; CALLING SEQUENCE: 
;    QuerySimbad, name, ra, dec, [ id, Found=, /CFA, /NED, ERRMSG=, /VERBOSE]
;
; INPUTS: 
...
;          The Strasbourg Simbad 4 server no longer (as of March 2009) seems
;          to be returning the primary ID, but the CFA server still does.
;
...
;-
 ...
 QueryURL = "http://" + base + "/nph-sesame/-o/SVN?" + $
  ...
  ;;
  if keyword_set(NED) THEN BEGIN
      if (strmid(result.text[6],1,3) NE 'PRE') and $
