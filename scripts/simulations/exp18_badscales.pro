outdir = getenv('WORKINGDIR')+'/artificial_sims/exp18_v1_badscales/'
file_mkdir,outdir
  ...
  for nn=0,0 do begin ; atmo amplitudes
    ...
    ;  for jj = 0,0 do begin
        ...
        for ii=0,0 do begin
          ...
          sigma_gp = 188.0 ; sigma-width of the Galactic Plane (can get more accurate value from Cara's paper)
          ...
          filename_prefix = 'exp18_ds2_astrosky_arrang'+string(array_angle,format='(I02)')+ $
          ...
          if remake_sim then begin
            make_artificial_timestreams,fakemap+0,hdr,ts=ts,astrosignal=astrosignal,bgps=bgps,mapstr=mapstr,$
              ...
              /dosave, array_angle=array_angle,stepsize=180,sample_interval=0.04
            seed1 = (jj+1)
            ...
            mem_iter,mapstr.outmap+"_preiter.sav",mapstr.outmap,/fromsave,niter=replicate(13,21),$
              ...
              fits_timestream=0,return_reconv=1,sim_input_ts=input_ts_ds2,/verbose
            ...
            mem_iter,mapstr.outmap+"_preiter.sav",mapstr.outmap,/fromsave,niter=replicate(13,21),$
              fits_out=[0,1,2,5,10,20],dosave=2,plot_all_timestreams=[3],plot_bolos=[7],plot_weights=1,$
              fits_timestream=0,return_reconv=1,sim_input_ts=input_ts_ds2,/verbose,do_weight=0
          ...
          exp12dir = getenv('WORKINGDIR')+"/artificial_sims/exp12_simple/"
          exp12fn = repstr(filename_prefix,'18','12')
          compare_images,v2filename,prefix2=exp12dir+exp12fn,prefix1=outmap,suffix2="_map20.fits",suffix1="_map20.fits",$
              output_name=outdir+"compare_exp18_exp12",in1='exp18',in2='exp12',/samescale,wcsaperture=" ",cuts='0.5,1.00'
