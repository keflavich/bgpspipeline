...
\title{The NEVERENDING Bolocam Galactic Plane Survey: Survey Description and Data
Reduction}
...
\subsection{Comparison to SCUBA Legacy Source Positions}
\label{sec:SCUBAPointingComparison}
%and SHARC-II Positions}

The same cross-correlation procedure was used to compare to the SCUBA
maps from the Legacy Catalog.  The morphological correlation between
SCUBA and BGPS sources is excellent, so cross-correlation maps could
be used to test our pointing model.

An example is shown in Figure \ref{fig:SCUBAPointingComparison}.

{\it Do the negative bowls of either survey bunge up the
cross-correlation?  If not, how do we deal with them?}
...
was constructed.  Notice the good sampling of the entire region used
for the Survey.}{fig:PointingCalibrators}{1.0}
% -----------------------------------------------------------------------------
...
distribution.]
{\label{fig:PointingModel-b}\includegraphics[scale=0.5]{pointing_model_residuals}}
...
kernel for 13 PCA components subtracted }{fig:PCA_Filter}{1.0}
