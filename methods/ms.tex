...
\ncores\ clumps to a limiting non-uniform 1-$\sig$ noise level in the range \bgpsdepthlow\ and
...
long-wavelength spectral band complementary to imminent submillimeter surveys
with SCUBA-2 and Herschel-SPIRE.
...
John, Jonathan)], in general, clumps are larger and less dense. Clumps can be
...
\begin{figure}
  \begin{minipage}{6.5in}
    \begin{center}
      ...
      \caption{$\lon=-10.5$ to $\lon=19.5$ Units are Jy/Beam.  The brightest
	sources, e.g. Sgr B2, Sgr A, and sources near $\lon=10$ and
	$\lon=13$, appear to be saturated, but this is only a display
	artifact.  The noise is more pronounced from $\lon=-7$ to $\lon=-2$
