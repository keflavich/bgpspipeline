...
\bgpsdepthhigh\ m\jyb RMS.  We present maps of 97 contiguous square degrees from
...
\begin{figure}
  ...
  \begin{minipage}{6.5in}
    \begin{center}
      ...
      \caption{$\lon=-10.5$ to $\lon=19.5$ Units are \jyb. 
%      The brightest
%	sources, e.g. Sgr B2, Sgr A, and sources near $\lon=10$ and
%	$\lon=13$, appear to be saturated, but this is only a display
%	artifact.
...
\begin{figure}
  \begin{minipage}{6.5in} 
    \begin{center}
      ...
      \caption{$\lon=19.5$ to $\lon=49.5$.  Units are \jyb .  G34.3+0.15, W
	51, W 43, W 49, and M 17 appear to be saturated, but this is
...
the bolometer signal time series under the assumption that the bulk of the
correlated signal is atmospheric (under this assumption, astrophysical extended
structures are treated as atmospheric noise).  The iterative estimation
...
to make the flux density measurements. [John - what does `scaled' mean?]
...
Motte Source &  Motte Table 1   &  Motte scaled  & BGPS  & Ratio &  Comments \\
      Number &  integrated flux &  \jyb          & \jyb  &       &           \\
}
...
response to the atmosphere for a single detector compared to the array median.
Outliers are from noisy scans that are strongly
downweighted.}{fig:CalibrationCurves}{1.0}
...
This results in a slightly blurred beam (35\farcs6 instead of 31\farcs2).
