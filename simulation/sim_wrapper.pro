function sim_wrapper,bgps,mapstr,nsources,mapcube=mapcube,niter=niter,noiselevel=noiselevel,$
    ...
    if keyword_set(jitter) then begin
        ...
        if jitter eq 1 then begin
            ...
            randx = reform( bgps.bolo_params[2,*] * cos(bgps.bolo_params[1,*]*!dtor) - bolo_params[2,*]/3600.*5*7.7 * cos(bolo_params[1,*]*!dtor) )
            randy = reform( bgps.bolo_params[2,*] * sin(bgps.bolo_params[1,*]*!dtor) - bolo_params[2,*]/3600.*5*7.7 * sin(bolo_params[1,*]*!dtor) )
        endif else begin
