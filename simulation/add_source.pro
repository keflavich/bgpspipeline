function add_source,blank_map,xcen,ycen,xsize,ysize,amplitude,angle,marspsf=marspsf
...
;    g = amplitude*exp(-(xx-xxcen)^2/(2*xsize^2)-(yy-yycen)^2/(2*ysize^2))
    ...
    if keyword_set(marspsf) then begin
        mars = readfits('/Volumes/disk3/adam_work/pointmaps/PSF_mars_crop_norm.fits',/silent)
        sizemars = size(mars,/dim)
        bm = blank_map
        bm[xcen-sizemars[0]/2:xcen+sizemars[0]/2,ycen-sizemars[1]/2:ycen+sizemars[1]/2] = mars/max(mars)*amplitude
    endif else begin
